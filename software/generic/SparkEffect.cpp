...
SparkEffect::SparkEffect(DeviceDescription *const device) : Effect(device) {}
...
CRGB SparkEffect::GetRGB(uint8_t ledIndex, uint32_t timeMs,
  ...
  int16_t pos =
      ((timeMs * (device->virtualLeds + pulse_size)) / 3000) % ((device->virtualLeds + pulse_size) * 2);

  bool reverse = false;
  if (pos > (device->virtualLeds + pulse_size)) {
    reverse = true;
    pos = (device->virtualLeds + pulse_size) - (pos - (device->virtualLeds + pulse_size));
  }
  ...
  if (relative_pos < 0) {
    hsv.v = 0;
  } else if (relative_pos < (device->virtualLeds + pulse_size)) {
