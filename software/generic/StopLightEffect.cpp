...
StopLightEffect::StopLightEffect(DeviceDescription *const device) : Effect(device) {}

CRGB StopLightEffect::GetRGB(uint8_t ledIndex, uint32_t timeMs,
                             RadioPacket *setEffectPacket) {
  const uint16_t led_pos = abs((device->virtualLeds >> 1) - ledIndex) << 8;
  const uint16_t segment = device->virtualLeds << (8 - 3);
  timeMs = timeMs >> 11;
